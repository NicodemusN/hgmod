...
public class ForgeSpawnEggItem extends SpawnEggItem
{
    ...
    public static SpawnEggItem fromEntityType(@Nullable EntityType<?> type)
    {
        ...
        return ret != null ? ret : SpawnEggItem.getEgg(type);
    }
    ...
    private static final DefaultDispenseItemBehavior DEFAULT_DISPENSE_BEHAVIOR = new DefaultDispenseItemBehavior()
    {
        protected ItemStack dispenseStack(IBlockSource source, ItemStack stack)
        {
            ...
            try
            {
                type.spawn(source.getWorld(), stack, null, source.getBlockPos().offset(face), SpawnReason.DISPENSER, face != Direction.UP, false);
            }
            catch (Exception exception)
            {
                DefaultDispenseItemBehavior.LOGGER.error("Error while dispensing spawn egg from dispenser at {}", source.getBlockPos(), exception);
